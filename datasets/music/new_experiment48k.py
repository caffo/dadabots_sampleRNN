import numpy as np
import sys, os, subprocess, random, time, glob, soundfile as sf
...
for i in xrange(int((length//8)*3)-1):
    time = (i * 8 )/ 3
    os.system('ffmpeg -ss {} -t 8 -i {}/preprocess_all_audio.wav -ac 1 -ab 16k -ar 48000 {}/p{}.flac'.format(time, OUTPUT_DIR, OUTPUT_DIR, i))
'''
size = 8
num = 6400
